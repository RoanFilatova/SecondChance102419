...
namespace DataAccessLayer
{
    public class DogMapper : Mapper
    {
        int OffsetToDogID; // 0
        int OffsetToName; // 1
        int OffsetToBreedID; //2
        int OffsetToIsSmallBreed; //3
        int OffsetToIsDogHairless; //4
        int OffsetToMedical; //5
        int OffsetToAdoptDate; //6
        int OffsetToSurrenderDate; //7
        ...
        public DogMapper(SqlDataReader reader)
        {
            ...
            Assert(7 == OffsetToSurrenderDate, $"SurrenderDate is {OffsetToSurrenderDate}, instead of 7 as expected");
        }
